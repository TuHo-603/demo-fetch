...
function renderItems(data) {
    ...
    let htmls = data.map((datas) => `
        <li>
            <h3>${datas.name}</h3>
            <p>${datas.quantity}</p>
            <button onclick="deleteItems(${datas.id})" name="delete">Delete</button>
        </li>
    `);
...
function delItem(data, callBack) {
    fetch(itemApi, {
        method: "DELETE",
        headers: {
            "Content-Type": "application/json"
...
function deleteItems(id) {
    let getButtonDelete = document.querySelector('button[name="delete"]');
    getButtonDelete.onclick = () => {
        delItem(id, () => {
            renderItems();
        });
    }
