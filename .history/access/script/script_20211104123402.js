...
function renderItems(data) {
    ...
    let htmls = data.map((datas) => `
        <li>
            ...
            <button onclick="repairItem(${datas.id})" name="repairItem">Repair</button>
        </li>
    `);
...
function repairItem(id, callBack) {
    fetch(itemApi + "/" + id, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
...
function handleRepair() {
    let getButtonRepair = document.querySelector('button[name="repairItem"]');
